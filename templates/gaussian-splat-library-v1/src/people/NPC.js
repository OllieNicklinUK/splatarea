...
const WANDER_SPEED = 0.9;  // faster wander so motion reads clearly at scale
...
export class NPC {
  constructor(scene, type, x, z, options = {}) {
    ...
    // Optional voxel mesh for wall avoidance (set via setVoxelMesh)
  ...
  update(delta, clawPos, clawClosed, options = {}) {
    ...
    // Voxel wall avoidance — cast a short ray forward; if it hits geometry,
    // turn away by ~90° with some randomness.
    if (this._voxelMesh) {
      const fwdDir = new THREE.Vector3(Math.sin(this.angle), 0, Math.cos(this.angle));
      const origin = new THREE.Vector3(
        this.group.position.x, this._floorY + 0.9, this.group.position.z
      );
      this._wallRc.set(origin, fwdDir);
    ...
    this.group.position.y = this._floorY; // stay on floor
