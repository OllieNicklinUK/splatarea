...
export class Vehicle {
	...
	update( dt, controlsInput ) {
		...
		if ( this.spherePos.y < - 10 ) this.resetToSpawn();
	...
	resetToSpawn() {
		...
		if ( this.rigidBody ) {
			...
			rigidBody.setPosition( this.physicsWorld, this.rigidBody, [ spawn.x, spawn.y, spawn.z ], false );
		...
		this.spherePos.copy( spawn );
		this.sphereVel.set( 0, 0, 0 );
		this.linearSpeed = 0;
		this.angularSpeed = 0;
		this.acceleration = 0;
		this.container.position.set( spawn.x, spawn.y - 0.5, spawn.z );
