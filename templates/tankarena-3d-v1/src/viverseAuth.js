...
export const VERSION_NAME = VIVERSE_CONFIG.versionName;
...
export class ViverseAuthController {
  constructor(onStateChange = () => {}) {
    ...
    this.state = {
      ...
      profile: null,
      error: "",
      appId: VIVERSE_CONFIG.appId
    };
    this.initPromise = null;
    console.log(`[TankArenaAuth] ${VERSION_NAME} bootstrap`);
  }
  ...
  async initialize() {
    ...
    this.initPromise = (async () => {
      ...
      try {
        ...
        if (!sdk) {
          this.setState({ status: "ready", error: "VIVERSE SDK not detected. Local play still works." });
          return null;
        }
        ...
        if (!VIVERSE_CONFIG.appId) {
          this.setState({
            status: "ready",
            error: "Missing VIVERSE App ID. Local play still works."
          });
          return null;
        }
        ...
        if (typeof ClientClass !== "function") {
          this.setState({
            status: "ready",
            error: "VIVERSE auth client unavailable. Local play still works."
          });
          return null;
        }
  ...
  async enrichProfile({ sdk, authClient, authResult, token }) {
    ...
    const resolvedSdk = sdk || this.state.sdk || detectSdkGlobal();
    ...
    if (token && typeof AvatarClass === "function") {
      try {
        const avatarClient = new AvatarClass({
          baseURL: VIVERSE_CONFIG.avatarBaseUrl,
          accessToken: token,
          token,
          authorization: token,
          appId: VIVERSE_CONFIG.appId,
    ...
    if (needsMoreProfile(mergedProfile) && typeof authClient?.getUserInfo === "function") {
      try {
        merge(await authClient.getUserInfo());
      } catch {}
    }

    if (needsMoreProfile(mergedProfile) && typeof authClient?.getUser === "function") {
      try {
        merge(await authClient.getUser());
      } catch {}
    }

    if (needsMoreProfile(mergedProfile) && typeof authClient?.getProfileByToken === "function") {
      try {
        merge(await authClient.getProfileByToken(token));
      } catch {}
    }

    const previewOrigin =
      typeof window !== "undefined" && /\.world\.viverse\.app$/i.test(window.location.hostname);
  ...
  async login() {
    const authClient = this.state.authClient;
    if (!authClient) {
