...
export class Vehicle {
	...
	// spawnOrigin: THREE.Vector3 — used as the reset position when car falls off
	constructor( spawnOrigin ) {

		this.spawnOrigin = spawnOrigin ? spawnOrigin.clone() : new THREE.Vector3( 3.5, 0.5, 5 );

		this.linearSpeed  = 0;
		this.angularSpeed = 0;
		this.acceleration = 0;

		this.spherePos = this.spawnOrigin.clone();
		...
		this.prevModelPos  = this.spawnOrigin.clone();
		this.prevModelPos.y = this.spawnOrigin.y - 0.5;
	...
	update( dt, controlsInput ) {
		...
		// Reset car if it falls far below spawn
		if ( this.spherePos.y < this.spawnOrigin.y - 15 ) {

			const o = this.spawnOrigin;

			if ( this.rigidBody ) {

				rigidBody.setPosition( this.physicsWorld, this.rigidBody, [ o.x, o.y, o.z ], false );
