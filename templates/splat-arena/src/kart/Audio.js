...
export class GameAudio {
	...
	init( camera ) {
		...
		loader.load( '/kart/audio/engine.ogg', ( buffer ) => {
		...
		loader.load( '/kart/audio/skid.ogg', ( buffer ) => {
		...
		loader.load( '/kart/audio/impact.ogg', ( buffer ) => {
	...
	update( dt, speed, throttle, driftIntensity ) {
		...
		this.engineLayerSound.setVolume( THREE.MathUtils.lerp( currentVol, targetVol, dt * 5 ) * 0.4 );
		...
		this.engineFilter.frequency.setTargetAtTime( targetCutoff, this.listener.context.currentTime, 0.05 );

		const shouldSkid = driftIntensity > 0.5;
		const skidVol = shouldSkid
			? remap( THREE.MathUtils.clamp( driftIntensity, 0.5, 2.5 ), 0.5, 2.5, 0.05, 0.3 )
			: 0;
