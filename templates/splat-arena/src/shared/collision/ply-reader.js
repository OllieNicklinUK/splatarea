...
// maxGaussians: cap on how many to keep — stride-sampled evenly across the file
// so the output array never exceeds maxGaussians * FLOATS_PER_GAUSSIAN * 4 bytes.
// Without this, a 10M-Gaussian file allocates ~640 MB of JS heap before filtering.
export function parseGaussians(plyBuffer, opacityThreshold = 0.2, maxGaussians = Infinity) {
    ...
    // Stride-sample so the output never exceeds maxGaussians entries.
    // Allocating count * FLOATS_PER_GAUSSIAN for a 10M-Gaussian file = 640 MB;
    // with stride sampling the allocation is capped at maxGaussians * 64 bytes.
    const sampleStride  = (maxGaussians < count) ? Math.max(1, Math.floor(count / maxGaussians)) : 1;
    const allocCount    = (sampleStride > 1) ? Math.min(count, maxGaussians) : count;
    const gaussians = new Float32Array(allocCount * FLOATS_PER_GAUSSIAN);
    let kept = 0;
...
export async function fetchAndParseGaussians(url, opacityThreshold = 0.2, onLog, maxGaussians = Infinity) {
    ...
    const result = parseGaussians(buf, opacityThreshold, maxGaussians);
    onLog?.(`${result.numGaussians.toLocaleString()} Gaussians sampled`);
    return result;
}
