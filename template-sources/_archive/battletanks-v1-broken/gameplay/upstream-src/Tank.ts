...
import { checkCollisionTankWithTank, checkCollisionTankWithWall } from "../../core-engine/upstream-src/utils/collision";
import { PBar } from "../../core-engine/upstream-src/utils/PBar";
...
class Tank extends MovableObject {
  ...
  private normalizeVisualScale(object: THREE.Object3D, targetLongestSide = 70) {
    const box = new THREE.Box3().setFromObject(object);
    const size = box.getSize(new THREE.Vector3());
    const longestSide = Math.max(size.x, size.y, size.z);
    if (!longestSide || !Number.isFinite(longestSide)) return;
    const scaleFactor = targetLongestSide / longestSide;
  ...
  constructor(name: string, tank_mesh: THREE.Object3D | null,
    ...
    if (tank_mesh != null) {
      const tankVisual = tank_mesh.clone(true);
      this.mesh.add(tankVisual);
      this.normalizeVisualScale(tankVisual, 70);
      ...
      this.mesh.children[0].rotation.z = name === "player2" ? 0 : Math.PI;


      this.mesh.castShadow = true;
      this.mesh.receiveShadow = true;
      this.mesh.position.set(name === "player2" ? 420 : -420, name === "player2" ? 420 : -420, 0);
    }
