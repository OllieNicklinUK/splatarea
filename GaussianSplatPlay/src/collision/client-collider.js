...
export async function generateCollision({
    ...
    try {
        ...
        if (plyBuffer) {
            ...
            parsed = parseGaussians(plyBuffer, opacityThreshold);
            onLog?.(`${parsed.numGaussians.toLocaleString()} Gaussians above opacity threshold`);
        } else if (plyUrl) {
            parsed = await fetchAndParseGaussians(plyUrl, opacityThreshold, onLog);
        } else {
        ...
        // Subsample very large splats — more Gaussians don't improve collision accuracy.
        // 400K gives adequate coverage for any scene size and cuts spatial-index
        // build + GPU dispatch time by ~35% vs the previous 600K cap.
        const MAX_GAUSSIANS = 400_000;
        let { gaussians, numGaussians, bounds } = parsed;
        if (numGaussians > MAX_GAUSSIANS) {
            onLog?.(`Subsampling ${numGaussians.toLocaleString()} → ${MAX_GAUSSIANS.toLocaleString()} Gaussians for voxelization`);
            const step = numGaussians / MAX_GAUSSIANS;
            const sub = new Float32Array(MAX_GAUSSIANS * 16);
            for (let i = 0; i < MAX_GAUSSIANS; i++) {
                const src = Math.floor(i * step) * 16;
                sub.set(gaussians.subarray(src, src + 16), i * 16);
            }
            gaussians = sub;
            numGaussians = MAX_GAUSSIANS;
        }
