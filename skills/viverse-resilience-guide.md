# VIVERSE Hub Resilience Guide (v5.4 - Deterministic Injection)
...
- **Deterministic Injection**: Use Vite's `define` property (e.g., `define: { 'process.env.VITE_VIVERSE_CLIENT_ID': JSON.stringify('ca4d46bnax') }`) to ensure App ID injection in environments where `.env` file persistence or template overrides are restricted.
- **Grep Gate**: `grep -r "YOUR_APP_ID" dist/assets/` MUST return 0 results after publish-time injection. Run `grep -r YOUR_APP_ID dist/` (all files) and fail if any remain.
- **Compiled JS injection**: The `sed` replacement MUST target `*.js` bundles in addition to `*.html` and `*.json`. Omitting `*.js` leaves `YOUR_APP_ID` in Vite-compiled chunks → SDK auth initializes with wrong ID → guest mode every time.
  Correct pattern: `find dist/ \( -name "*.json" -o -name "*.html" -o -name "*.js" \) | xargs sed -i "" "s/YOUR_APP_ID/<AppId>/g"`
- **Stale hardcoded IDs in index.html**: Templates must NOT ship with a real app ID in `window.__APP_CONFIG__` or equivalent runtime config blocks in `index.html`. Use `YOUR_APP_ID` as the placeholder. A stale dev ID (e.g. `clientId: "abc123"`) will survive sed replacement undetected and override auth — always guest mode, no grep-gate alarm.
